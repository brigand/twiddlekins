...
namespace robokins.Trigger
{
	...
	/// </returns>
	delegate bool TriggerHandleDelegate(ReceivedMessage msg, TriggerResponse resp);
	...
	partial class TriggerAction
	{
		...
		public TriggerAction(string search, 
		                     string match, string starts, string ends, 
		                     bool enc, string _returnformat, int requiredarguments)
		{
			searchUri = search;
			matchString = match;
			startsWith = starts;
